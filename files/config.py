...
# 평가 유니버스 규모(코어). 롱테일은 별도 큐레이션 리스트가 추가됨.
UNIVERSE_KR_N = 200    # 네이버 코스피 시총 상위
UNIVERSE_US_N = 600    # S&P500 + 보충
...
# ---------------------------------------------------------------------------
# 롱테일(신흥 강자) 발굴 — 우량주 대신 중소형·신흥 종목을 공격적으로 탐색
#   별도 유니버스 + 공격적 가중치로 시장별 3종목 추가 선정(pick_type="longtail").
# ---------------------------------------------------------------------------
LONGTAIL_N = 3

# 가중치 '부분 오버라이드'(미지정 키는 CRITERIA 기본값 사용).
#   모멘텀·테마·이벤트(추정치/호재) 고가중, 대형 가치·품질 저가중.
LONGTAIL_WEIGHTS = {
    "mom_12_1":     1.6,   # 신흥 모멘텀 핵심
    "theme":        1.5,   # 테마 주도 급등
    "est_revision": 1.2,   # 추정치 상향(재평가 초기)
    "rumor":        1.1,   # 투기적 호재 허용(고위험)
    "quant":        1.0,
    "beneficiary":  1.0,
    "insider":      0.9,
    "target_gap":   0.8,
    "blog":         0.7,
    "governance":   0.4,
    # 대형 우량 성격 팩터는 낮춤(신흥주는 아직 저PER·고품질이 아님)
    "undervalued":  0.3,
    "earnings":     0.4,
    "fscore":       0.4,
    "fcf_yield":    0.3,
...
class Thresholds:
    ...
    # 총주주환원율/현금배당성향(%): 이 값 이상이면 주주환원 우수 (배당성향은 통상 20~40%)
    PAYOUT_GOOD = 30.0
