...
#
#    recommendations.json and cache/ are always regenerated by the engine, so
#    we discard any local modifications before pulling — no need to stash them.
#    Other files (.claude/settings.local.json etc.) are kept via --autostash,
#    but recommendations.json is explicitly reset so stash-pop never conflicts.
git checkout HEAD -- recommendations.json 2>&1 |
    Out-File -FilePath $log -Append -Encoding utf8
